...
import { Search, MapPin, ChevronRight, Loader2, X, Star, Snowflake, Building2, SlidersHorizontal, Plus, Wrench, Wind, Thermometer } from 'lucide-react';
...
function SearchResults() {
  ...
  const facilityTypes = [
    { value: 'all', label: 'Alle Types', icon: Snowflake },
    { value: 'airco-installateur', label: 'Airco Installateur', icon: Wrench },
    { value: 'warmtepomp-specialist', label: 'Warmtepomp Specialist', icon: Wind },
    { value: 'klimaattechniek-bedrijf', label: 'Klimaattechniek', icon: Thermometer },
    { value: 'airco-onderhoudsbedrijf', label: 'Onderhoudsbedrijf', icon: Building2 },
  ];
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Hero Header */}
      <div className="bg-gradient-to-br from-cyan-600 via-cyan-700 to-cyan-800 text-white py-12">
        <div className="container mx-auto px-4">
          <h1 className="font-serif text-3xl sm:text-4xl font-semibold mb-4 text-center">
            {searchQuery ? `Resultaten voor "${searchQuery}"` : 'Vind Airco Monteurs'}
          </h1>
          <p className="text-cyan-100 text-center max-w-2xl mx-auto">
            Doorzoek ons netwerk van airco installateurs en klimaatspecialisten in Nederland
          </p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-7xl mx-auto">
          {/* Search & Filter Bar */}
          <Card className="p-4 mb-8 shadow-soft border-cyan-100">
            <div className="flex flex-col lg:flex-row gap-4">
              {/* Search Input */}
              <div className="flex-1 relative">
                <MapPin className="absolute left-4 top-1/2 transform -translate-y-1/2 text-cyan-600 w-5 h-5" />
                <Input
                  type="text"
                  ...
                  className="pl-12 h-12 text-base focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500"
                />
              </div>
              ...
              <div className="hidden lg:flex items-center gap-3">
                <Select value={selectedType} onValueChange={(val) => {
                  ...
                  <SelectTrigger className="w-[200px] h-12">
                    <SelectValue placeholder="Type Bedrijf" />
                  </SelectTrigger>
                  <SelectContent>
                ...
                <Button onClick={handleSearch} size="lg" className="h-12 px-6 bg-cyan-600 hover:bg-cyan-700 text-white">
              ...
              <div className="lg:hidden flex gap-3">
                ...
                >
                  ...
                  Filters
                  {hasActiveFilters && (
                    <span className="ml-2 w-5 h-5 bg-cyan-600 text-white rounded-full text-xs flex items-center justify-center">
                      !
                    </span>
                  )}
                </Button>
                <Button onClick={handleSearch} className="h-12 px-6 bg-cyan-600 hover:bg-cyan-700 text-white">
          ...
          {hasActiveFilters && (
            <div className="flex flex-wrap items-center gap-2 mb-6">
              ...
              {searchQuery && (
                <span className="inline-flex items-center gap-1 px-3 py-1 bg-cyan-50 text-cyan-700 border border-cyan-200 rounded-full text-sm">
          ...
          <div className="mb-6">
            {loading ? (
              ...
              <p className="text-muted-foreground">
                <span className="inline-flex items-center justify-center min-w-[2rem] px-2 py-0.5 bg-cyan-600 text-white text-sm font-semibold rounded-full mr-2">{results.length}</span>
                monteurs gevonden
              </p>
            )}
          </div>
          ...
          {loading ? (
            <div className="flex justify-center py-20">
              <div className="text-center">
                ...
                <p className="text-muted-foreground">Monteurs zoeken...</p>
              </div>
            </div>
          ) : results.length === 0 ? (
            <Card className="p-8 sm:p-12 text-center border-cyan-100">
              ...
              <p className="text-muted-foreground mb-6 max-w-md mx-auto">
                We konden geen monteurs vinden die aan je zoekopdracht voldoen.
                Probeer andere zoektermen of pas de filters aan.
              </p>
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                <Button variant="outline" onClick={clearFilters} className="border-cyan-200 text-cyan-700 hover:bg-cyan-50">
                  Filters Wissen
                </Button>
              </div>
            </Card>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                {results.slice(0, visibleResults).map((facility, index) => (
                  <Fragment key={facility.slug}>
                    <Link
                      href={`/monteur/${facility.slug}`}
                      className="group"
                    >
                      <Card className="h-full overflow-hidden border-2 border-transparent hover:border-cyan-300 hover:shadow-lg transition-all duration-300">
                        <div className="p-6">
                          <div className="flex items-start justify-between mb-3">
                            <h3 className="font-serif text-lg font-semibold text-foreground group-hover:text-cyan-600 transition-colors line-clamp-2">
                              {facility.name}
                            </h3>
                          ...
                          <div className="flex items-center justify-between pt-3 border-t">
                            <span className="text-xs font-medium bg-cyan-50 text-cyan-700 px-2.5 py-1 rounded-full capitalize">
                              {facility.type?.replace(/-/g, ' ') || 'Airco Monteur'}
                            </span>
          ...
          {/* Browse by Type */}
          <section className="mt-16 pt-8 border-t border-cyan-100">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {facilityTypes.slice(1).map((type) => (
                ...
                >
                  <Card className="p-5 h-full text-center hover:shadow-lg hover:border-cyan-300 transition-all duration-300 group">
                    <div className="w-12 h-12 rounded-xl bg-cyan-50 text-cyan-600 flex items-center justify-center mx-auto mb-3 group-hover:bg-cyan-600 group-hover:text-white transition-colors">
                      <type.icon className="w-6 h-6" />
                    </div>
                    <h3 className="font-medium text-sm mb-1">{type.label}</h3>
                    <p className="text-xs text-cyan-600 group-hover:text-cyan-700">
                      Bekijk alle
                    </p>
                  </Card>
                </Link>
...
export default function SearchPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-background">
        <div className="bg-gradient-to-br from-cyan-600 via-cyan-700 to-cyan-800 py-12">
